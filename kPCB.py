...
def get_data_generators(images_path, img_size):
    datagenTraining = ImageDataGenerator(validation_split=0.2, rescale=1.0 / 255)
    datagenValidation = ImageDataGenerator(validation_split=0.2, rescale=1.0 / 255)
...
def model_predict(model, data, model_name="Model"):
    ...
    for i, k in enumerate(preds):
        ...
        df = df.append(
            pd.DataFrame(r, columns=["Found", "Expected", "Identified", "File"])
        )
...
def run_model_1(
    epochs, batch, outputs, img_size, train_generator, val_generator, verb=1
):
    model = Sequential()
...
def run_model_pretrained_xception(
    epochs, outputs, img_size, train_generator, val_generator, verb=1
):
    ...
    history = model.fit(
        train_generator, epochs=epochs, validation_data=val_generator, verbose=verb
    )

    return model, history


def run_model_pretrained_mobilenetv2(
    epochs, outputs, img_size, train_generator, val_generator, verb=1
):
...
train_gen, val_gen = get_data_generators_randomized(
    img_size=img_size, images_path=train_path
)
...
m, h = run_model_1(epochs, batch, outputs, img_size, train_gen, val_gen, 1)
print_quick_stats(m, val_gen)
plot_history(h)
